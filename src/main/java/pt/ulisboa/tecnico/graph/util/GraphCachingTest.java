	...
	public static void main(final String[] args)
    {
		...
		if(runningRemoteFlink) {
			...
			try {
				...
				mvnRootFile = new File(GraphCachingTest.class.getProtectionDomain()
						.getCodeSource()
						.getLocation()
						.getPath()
						.replaceAll("%20", " "))
...
*/
		...
		unoptimizedLoop(updateSize, 5, graph, env);
    }
	...
	private static void optimizedAccumulationLoop(final int updateSize, Graph<Long, NullValue, NullValue> graph, final ExecutionEnvironment env) {
		try (final BufferedReader br = new BufferedReader(new FileReader(streamPath))) {
			...
			final HashMap<Long, String> vids = new HashMap<Long, String>();
			final HashMap<Long, String> eids = new HashMap<Long, String>();
	...
	// Flink iteration that doesn't work. (DOES NOT WRITE TO SINKS DURING DELTA OR BULK ITERATIONS)
	...
	// Java iteration with accumulated operator execution (BAD PERFORMANCE - ALWAYS EXECUTES EVERYTHING FROM START)
	private static void unoptimizedLoop(final int updateSize, final int executionLimit, Graph<Long, NullValue, NullValue> graph, final ExecutionEnvironment env) {
		try (final BufferedReader br = new BufferedReader(new FileReader(streamPath))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if(updates.size() == updateSize) {
					...
					//final DataSet<Tuple2<Long, Double>> ranks = graph.run(new SimplePageRank<>(beta, previousRanks, iterations));
					final DataSet<Tuple2<Long, Double>> ranks = newGraph.run(new SimplePageRank<>(beta, previousRanks, iterations));
